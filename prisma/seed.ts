import { databaseClient } from '../packages/database/src';
...
async function main() {
  ...
  const serviceRoutes = await Promise.all([
    databaseClient.serviceRoute.upsert({
      where: { id: 1 },
      update: {
        serviceId: '1',
        order: 0,
  ...
  const serviceCustomFields = await Promise.all([
    databaseClient.serviceCustomField.upsert({
      where: { id: 1 },
      update: {
        serviceId: '1',
        path: 'browser',
  ...
  const incidentTypes = await Promise.all([
    databaseClient.incidentType.upsert({
      where: { id: 1 },
      update: {
        name: '网站崩溃',
        serviceId: '1',
  ...
  const severityConditions = await Promise.all([
    // 网站崩溃的严重程度条件
    databaseClient.incidentTypeSeverityCondition.upsert({
      where: { id: 1 },
      update: {
        severity: 'CRITICAL',
        incidentTypeId: 1,
  ...
  const globalCustomFields = await Promise.all([
    databaseClient.globalCustomField.upsert({
      where: { id: 1 },
      update: {
        path: 'timestamp',
        type: 'DATE',
